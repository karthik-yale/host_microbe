...
class phenotype_constraint():
    ...
    def rel_BCD_vs_phenotype(self, phenotype, plot=True, return_constraint=False):
        ...
        if plot:
            x = np.linspace(min(met_value_list), max(met_value_list), 100)
            plt.plot(x, np.ones(100), ':', color='black')
            plt.errorbar(met_value_list, bcd_mean_mean_list, bcd_std_list, color='blue')

            plt.ylim([0,1.2])
            plt.yticks([0,0.5,1])
            plt.xlabel('{} z-score'.format(phenotype))
    ...
    def phenotype_constraint_list(self):
        self.constraint_level_list = []
        for i in tqdm(range(10)):
            constraint_level = Parallel(n_jobs=-1)(
    ...
    # def bcd_constrained_by_meta(self, phenotype_list):
    #     '''
    ...
    #         while(len(pred_comp_list) < 50 and counter < 1000): #Makes sure we get at least 50 generated data points to make a prediction
    #             self.create_in_silico_samples()
    ...
    #                 pred_comp_list.append(self.gm_data[within_range_indices])
    #             counter += 1

    #         if len(pred_comp_list) == 0:
    #             raise ValueError('No matching points found. Increase threshold or sample more points')

    #         no_of_preds.append(len(pred_comp_list))
    #         pred_comp_list = np.array(pred_comp_list)
    #         pred_comp = np.mean(pred_comp_list, axis=0)
    #         bcd_list.append(bcd(pred_comp, data_point))
